...
//constructor
  ...
  function selclientes($filtro){
      
      if($this->con->conectar()==true){
          
          $consulta= "select identificador, nombre from v_cliente where $filtro";
            return  $this->con->consulta($consulta);
      }
  }
  ...
  function buscarclienteusuario($filtro){
      ...
      if($this->con->conectar()==true){
          ...
          $consulta= "select * from v_clientes_x_usuario where $filtro ORDER BY nombres";
          return  $this->con->consulta($consulta);
      }
  }
  ...
  function insertabloquep($info){
      ...
      if($this->con->conectar()==true){
          /////Modificacion sebastian cambio estado el dia 23/12/22
          $consulta= "INSERT INTO enc_bloque_preguntas (identificador, descripcion, tipo_estado, estado) VALUES( '$info[0]', '$info[1]', '$info[2]', '1')";
          return  $this->con->consulta($consulta);
      }
  }
...
function buscarespuestas($filtro){
    ...
    if($this->con->conectar()==true){
        ...
        $consulta= "select identificador, descripcion , vdom_tipo_dato, estado, tipo_estado from enc_respuestas where $filtro";
        return  $this->con->consulta($consulta);
    }
}
...
function buscapadres($filtro){
    ...
    if($this->con->conectar()==true){
        ...
        $consulta= "select codigo,identificador, nombre from enc_preguntas where $filtro";
        return  $this->con->consulta($consulta);
    }
}
...
function buscaenclistavalores($filtro){
    ...
    if($this->con->conectar()==true){
        ...
        $consulta= "select identificador, valor_alfa_numerico, estado from enc_lista_valores where $filtro";
         return  $this->con->consulta($consulta);
    }
}
...
function buscarbloquepregunta($filtro){
    ...
    if($this->con->conectar()==true){
        ...
        $consulta= "select obligatorio_informe,id_bien,(SELECT descripcion FROM cg_valores_dominio WHERE identificador=id_bien) desp_bien,identificador, descripcion, tipo_estado, estado,prioridad from enc_bloque_preguntas where $filtro AND estado=1";
        return  $this->con->consulta($consulta);
    }
}

function buscarbloquepreguntaB($filtro){
...
function buscarbloquepregunta2($filtro){
...
function buscarpreguntas($filtro){
    ...
    if($this->con->conectar()==true){
        ...
        $consulta= "select p.codigo,p.identificador, p.nombre, p.id_pregunta_padre padre, p.id_valor_res_cierre respcierre, p.id_valor_resp_activa_riesgo respriesgo , p.fecha_efectiva, p.estado, p.id_respuesta, r.descripcion nomrespuesta,p.id_tipo_informe,p.texto_informe,p.ayuda
...
function buscariesgopreg($filtro){
    ...
    if($this->con->conectar()==true){
        ...
        $consulta= "select DISTINCT r.id_riesgo id_riesgo , v.nombre nombre from enc_riesgo_a_activar r JOIN v_riesgos v ON v.id_riesgo = r.id_riesgo AND r.estado = 1 WHERE $filtro";
        return  $this->con->consulta($consulta);
    }
}

function selclientesfil($filtro){
...
function actualizapreg($filtro, $info){
    
    if($this->con->conectar()==true){
        
        $consulta= "update enc_preguntas set $info where $filtro";
        return  $this->con->consulta($consulta);
    }
}

function borrarriesgopreg($filtro) {
 
    if($this->con->conectar()==true){      
            $consulta= "delete from enc_riesgo_a_activar where $filtro";
            return  $this->con->consulta($consulta);
        }
}
...
function actualizabloquep($filtro, $info){
    
    if($this->con->conectar()==true){
        
        $consulta= "update enc_bloque_preguntas set $info where $filtro";
        return  $this->con->consulta($consulta);
    }
}
