...
$tipodocumentosolicitante = $_POST['tipodocumento'];
...
$consultainformacioninspeccion = $mysqli->query("SELECT * FROM enc_inspeccion WHERE identificador ='$identificador'");
$extraerInformacioinspeccion = $consultainformacioninspeccion->fetch_array(MYSQLI_ASSOC);
$numeroInspeccion = $extraerInformacioinspeccion['numero_inspeccion'];
$nombresolicitante = $extraerInformacioinspeccion['nombre_solicita'];
$fecha_solicitud = $extraerInformacioinspeccion['fecha_solicitud'];
//$longitud = $extraerInformacioinspeccion['longitud'];
//$latitud = $extraerInformacioinspeccion['latitud'];
...
$texto_informe = $mysqli->query("SELECT p_informe_completo($identificador) AS p_informe_completo");
$extraerinformetextocompleto = $texto_informe->fetch_array(MYSQLI_ASSOC);
$informe_texto = $extraerinformetextocompleto['p_informe_completo'];
...
$consultaimagenes = $mysqli->query("SELECT DI.id_inspeccion,DI.archivos,IM.pie_de_imagen,IM.archivo FROM enc_detalles_inspeccion DI,enc_imagenes_inspeccion IM WHERE DI.id_inspeccion = '$identificador' GROUP BY archivos ORDER BY DI.archivos ASC;");
while ($extraerimagen = $consultaimagenes->fetch_array()) {
    $imagen = $extraerimagen['archivos'];
...
if ($resultgrafica2->num_rows > 0) {
    while ($row = $resultgrafica2->fetch_assoc()) {
        $grafica_data2[] = [
            'label' => utf8_encode($row['riesgo']),
            'data' => [(int)$row['nr']],
            'backgroundColor' => $row['color_nr']
        ];
    }
} else {
    $grafica_data2[] = [
...
$tabla .= "   <td class='' style='background-color:#00E0FF' colspan='3' rowspan='$textoC'>
                <h3>
...
while ($extraerDatosdescripcioncolores = $consultavaloresdescripcioncolores->fetch_array()) {
    $ColorRiesgo .= '<tr>
        <th><button type="" style="background-color:' . $extraerDatosdescripcioncolores['codigo'] . '">' . $identificadorI = $extraerDatosdescripcioncolores['descripcion'] . '</button></th>
        <th><div class="campos_f" style="text-align:left;">' . $identificadorI = ' ' . $extraerDatosdescripcioncolores['descripcion_texto_cr'] . '</div></th>
        </tr>';
}
...
include 'MostrarPDF/indexfree.php';

$html = ob_get_clean();

$html = str_replace('%inspeccion%', $numeroInspeccion, $html);
...
$html = str_replace('%texto_informe%', $informe_texto, $html);
