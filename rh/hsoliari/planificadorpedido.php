...
?>
  ...
  // busco numero de habitaciones ocupadas
               ...
               echo '
                 <div class="ocupado">
                 <span>Productos Bajos de STOCK</span> <br>
...
if(!empty($_POST["guardar"])) {
 ...
 else
 {
   ...
   if (!isset($_POST['idtarifa1']) or (!isset($_POST['habit'])) or (!isset($_POST['idhab'])))
                    $mensaje = "Verifique el numero que la asignación de una habitación y escoger una tarifa.";
   else
   {
...
?>
  ...
  <FORM name="frmProduct" method="post" action="planificadorpedido.php" id="checkin">

    <div class="mt-5">
    <h3> Detalles del Planificador de Pedidos No. <?php  $XNUMRES ?> </h3>
    <label style="width:100%" for="status"> Estado del Pedido
<?php
...
?>
  ...
  <div>
           <label for="entrada">Periodo Inicial</label>
            <?php echo $hoy; ?>
           <label for="entrada">Periodo Final</label>
  ...
  <div class="viaje">
    <div >
          ...
          <input class="search_query form-control" type="text" name="cedula1" id="key" value="No asignado en este momento"
 ...
 <div class="op-b">
     ...
     <hr>
      <div class="mt-6">
      <h3 class="det-2" > Productos Bajos en Stock en el periodo categoria Frutas y Verduras </h3>
...
if ((isset($_POST["pax1"])) or (isset($pax1)))
{
      if (!isset($pax2))
       $pax2 = $pax1;
     $XHAB=$XOBJRES->buscar_productos();
     $clase="tipo-h2";
     $ruta = "pagos/files/products/";
     while($XHABRES=$XOBJRES->obtener_fila($XHAB))
     {
        ...
        $salida2 = date ( 'Y-m-d' , $salida2 );
              ...
              $tiha = $tiha. '<div><img src="'. $ruta. $XHABRES[1].'">
               <div><input type="checkbox" id="habit" name="habit" value="'. $XHABRES[0] . '" required>'. $XHABRES[0] . '</div>
               <div class="'.$clase.'">Unidad </div> Libra
               <div class="'.$clase.'">Cantidad </div> <input type="text" id="cant" name="cant" value="1"></div>';
...
<input class="btn btn-primary" type="submit" name="exportar" value="Exportar" />
