...
{?>
      ...
      <div class="row">
         <ul id="storeline">
           <?php if($this->user->role !== 'admin' && $this->user->role !== 'sales') { ?>
             ...
             <a <?php $store->status == 1 ? "" : 'style="pointer-events: none; display: inline-block;opacity: 0.3;"';?> href="javascript:void(0)"  onclick="OpenRegister(<?=$store->status ? $store->status : 0;?>, <?=$store->id;?>, '<?=$this->user->role;?>')">
               <li class="listing clearfix">
                 <div class="image_wrapper">
                   <img src="<?php base_url()?>assets/img/store.svg" alt="store">
                 </div>
                 <div class="info">
                   <span class="store_title"><?php $store->name;?></span>
                   <span class="store_info"><?php $store->city;?> <span>&bull;</span> <?=$store->phone;?> <span>&bull;</span> <?=$store->email;?></span>
                 </div>
                 <span class="store_type <?php $store->status == 1 ? 'store_open' : 'store_close';?>"><?= $store->status == 1 ? label('open') : label('close');?></span>
               </li>
             </a>
             <?php } ?>
             <?php endforeach;?>
   ...
   {?>
      ...
      <div class="container">
         ...
         <?php !$zones ? '<h4 style="margin-top:60px">'.label("NoTables").'</h4>' : '';?>
         <?php foreach ($zones as $zone):?>
         <div class="row">
            <h1 class="choose_store"> <?php $zone->name;?> </h1><hr>
         </div>
         <div class="row tablesrow">
            ...
            <div class="col-sm-2 col-xs-4 tableList">
               <?php if($table->time != ''){?><span class="tabletime"><?=$table->time;?></span><?php } ?>
                  <a href="pos/selectTable/<?php $table->id;?>">
                       <img src="<?php base_url()?>assets/img/<?= $table->status == 1 ? 'tableB'.rand(1, 6).'.svg' : 'table.svg'; ?>" alt="store">
                       <h2><?php $table->name;?></h2>
                  </a>
            </div>
            <?php } ?>
            <?php endforeach;?>
...
<div class="container-fluid">
   ...
   <div class="row">
      ...
      <div class="col-md-5 left-side">
         ...
         <div class="col-sm-6">
            <select class="js-select-options form-control" id="customerSelect" name="custid">
              <option value="0"><?=label("WalkinCustomer");?></option>
              <?php foreach ($customers as $customer):?>
                 <option value="<?=$customer->id;?>" ><?=$customer->name;?> / <?=$customer->numdocu;?></option>
              <?php endforeach;?>
            </select>
            <span class="hidden" id="customerS"></span>
            </div>
         <div class="col-sm-6">
            <span>     <input type="text" id="clientid" name="clientid" >
            </span>
         </div>
         ...
         <div class="col-xs-3 table-header nopadding">
            <h3 class="text-left"><?=label("Cantidad");?></h3>
         </div>
          <div class="col-xs-2 table-header nopadding">
            <h3><?=label("Impuestos");?></h3>
         </div>
         <div class="col-xs-2 table-header nopadding">
            <h3><?=label("Total");?></h3>
         </div>
            ...
            <div class="table-responsive col-sm-12 totalTab">
               <table class="table">
                  <tr>
                     ...
                     <td class="whiteBg" width="60%"><span id="Subtot"></span> <span id="Subtot1"></span><?=$this->setting->currency?>
                        <span class="float-right"><b id="ItemsNum"><span></span> <?=label("item");?></b></span>
                         <span class="float-right"><b id="totalitem"><span></b></span>
                     </td>
                  </tr>
                  <tr>
      ...
      <div class="col-md-7 right-side nopadding">
         <div class="row row-horizon">
                  ...
                  <?php
                  // Selecciono los diferentes amibientes
                  foreach ($Stores as $store):
                  ?>
                     <span class="categories" id="<?php $store->id; ?>"> <?php echo $store->name;?></span>
                  <?php
                     endforeach;?>
          </div>

              <!-- selecciono los consuos a las habitaciones -->
         <div id="productList2">
...
$(document).ready(function() {
    $('.holdList').load("<?php echo site_url('pos/holdList/'.$this->register)?>", function(){
      ...
      $('#customerS').load("<?php echo site_url('pos/CustomerName')?>/"+holdi, function(){
         ...
         if(res>0) {$('#customerSelect').val(res).trigger("change");
                $('#clientid').val(res);
                $('#clientid').text(res);
          }else{$('#customerSelect').val(0).trigger("change");}
      });
   });
  ...
  if(id === '0') {
     ...
     $.ajax({
         ...
         success: function(data)
         {
            ...
            $('.Remise').val(values[0]);
            $('#productList').load("<?php echo site_url('pos/load_posales')?>");
               $('#Subtot1').text(values[4]);
               $('#total').text(values[2]);
                $('#taxValue').text(values[3]);
              $('#totalitem').text(values[5]);
                 $('#productList').load("<?php echo site_url('pos/load_sales')?>/"+id);
                 $('#productList2').load("<?php echo site_url('pos/load_sales2')?>/"+id);
...
function saleBtn(type) {
   ...
   var Discount = $('#RemiseValue').text();
   var Subtotal = $('#Subtot1').text();
   var Total = $('#total').text();
   var createdBy = '<?php echo $this->user->firstname." ".$this->user->lastname;?>';
   var totalItems = $('#totalitem span').text();
   var Paid = $('#Paid').val();
   ...
   var taxamount =  parseFloat($('#taxValue').text());
   var discountamount = parseFloat($('#RemiseValue').text());
   var clientpaid = clientID+'~'+paidMethod;

  $.ajax({
      url : "<?php echo site_url('pos/AddNewSale')?>/"+clientpaid,
      type: "POST",
      ...
      success: function(data)
      {
         ...
         $('#productList').load("<?php echo site_url('pos/load_sales')?>"+clientID);
         $('#Subtot').load("<?php echo site_url('pos/subtot')?>", null, total_change);
         $('#AddSale').modal('hide');
         $('#ticket').modal('show');
