            ...
            <div class="table-responsive col-sm-12 totalTab">
               <table class="table">
                  ...
                  <tr>
                     ...
                     <td class="whiteBg"><input type="text" value="0.00" onchange="total_change()" id="<?=strval($this->setting->keyboard) === '1' ? 'num02' : ''?>" class="total-input Remise" placeholder="N/A"  maxlength="8">
                        <span class="float-right"><b id="abono"></b></span>
                     </td>
                  </tr>
                  <tr>
...
$(document).ready(function() {
   ...
   $('#Subtot').load("<?php echo site_url('pos/subtot')?>", null, total_change);
    $('#abonos').load("<?php echo site_url('pos/abonos')?>", null, total_change);
...
function add_posale(id)
{
   ...
   var waiterID = $('#WaiterName').find('option:selected').val();
     ...
     $.ajax({
         ...
         error: function (jqXHR, textStatus, errorThrown)
         {
            alert("error adicionando la venta pos");
         }
     });
...
function addPoptions()
{
   ...
   $.ajax({
       ...
       error: function (jqXHR, textStatus, errorThrown)
       {
          alert("error adicionando po options");
       }
   });
}
...
$("#customerSelect").change(function(){
  ...
  if(id === '0') {
     ...
     $.ajax({
         ...
         error: function (jqXHR, textStatus, errorThrown)
         {
            alert("error selecionando cliente");
         }
    });
 }
});
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
           ...
           <div class="form-group">
             ...
             <div class="form-group">
             <h3 id="abonos"></h2> <span></span><?php $abonos; ?> <span> Abonos Anteriores </span></h3>
          </div>
