...
class Pos extends CI_Controller
{
    ...
    public function selectTable($id,$clientid)
    {
    ...
    public function abonos()
    {
        ...
        foreach ($posales as $posale) {
            $sub += $posale->price * $posale->qt;
            // busco los abonos en tabla payments
           $payements = Payement::find('all', array('conditions' => array('sale_id = ?', $posale->sales_id)));
           if($payements){
             if (!isset($abonos))
                $abonos = 0;
             foreach ($payements as $pay) {
                 $abonos = $abonos +  number_format((float)$pay->paid, $this->setting->decimals, '.', '');
             }

            }
        }
        echo number_format((float)$abonos, $this->setting->decimals, '.', '');
    }
    // fin lmrg
    ...
    /*   $estadores = '13';
      ...
      $XOBJRES=new greservas;
        $sale1 = Sale::find( $posale->sales_id);
      if (isset($sale1->bookingid))
         $bookingid = $sale1->bookingid;
      else
         $bookingid = 0;
      if (isset($sale1->checkinid))
         $checkinid = $sale1->checkinid;
      else
         $checkinid = "";                        */

    /*  $XACTCALENDAR=$XOBJRES->actualiza_reservasale($bookingid, $estadores);
        $XACTCALENDAR=$XOBJRES->actualiza_calendardetsale($bookingid, $estadores);
       $XACTCALENDAR=$XOBJRES->actualiza_checkinsale($checkinid, $estadores);            */
        ...
        if (empty($holds))
            ...
            if (empty($posale)) {
                ...
                foreach ($holds as $hold) {
                    if (++ $i === $numItems)
                        $Tholds .= '<span class="Hold selectedHold" id="' . $hold->number . '"  onclick="SelectHold(' . $hold->number . ')">' . $hold->number . '<span id="Time">' . $hold->time . '#hab.'. $hold->numhab . '</span></span>';
                    else
                        $Tholds .= '<span class="Hold" id="' . $hold->number . '"  onclick="SelectHold(' . $hold->number . ')">' . $hold->number . '<span id="Time">' . $hold->time . '#hab.'. $hold->numhab . '</span></span>';
                }
            } else {
                foreach ($holds as $hold) {
                    if ($hold->number == $posale->number)
                        ...
                        $selected = '';
                    $Tholds .= '<span class="Hold ' . $selected . '" id="' . $hold->number . '" onclick="SelectHold(' . $hold->number . ')">' . $hold->number . '<span id="Time">' . $hold->time . '#hab.' . $hold->numhab . '</span></span>';
                }
            }
        }
        echo $Tholds;
