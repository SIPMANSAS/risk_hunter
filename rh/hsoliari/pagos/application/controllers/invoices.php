...
class Invoices extends CI_Controller
{
    ...
    public function Edit_Ajax($id)
    {
        ...
        $content = '<div class="row"><div class="col-md-12"><h4><b>' . label("Total") . '</b> ' . $sale->total . ' '
        . $this->setting->currency . ' <b>&emsp;' . label("Paid") . ' :</b> ' . $sale->paid . ' ' . $this->setting->currency
         . '<b> &emsp; Saldo  :</b> ' . ($sale->total - $sale->paid) . ' ' . $this->setting->currency .
         '</h4><div class="form-group"><label for="subtotal"> Cambiar Subtotal Tarifa </label> ';

         $content .= ' <input type="text" name="subtotal" id="customerSelect" value="' . $sale->subtotal . '" maxlength="12" >';


        $content .= '<div class="form-group"><label for="tax"> Cambiar Iva <span class="' . $satus . '">'
        . label($satus) . '<span></label>';

         $content .= ' &emsp; <input type="text" name="tax" id="changeStatus" value="' . $sale->tax . '" maxlength="12" > </div>';


         $content .= '<div class="form-group"><label for="total"> Cambiar Total Tarifa </label>';
          $content .= ' &emsp; <input type="text" name="total" id="total" value="' . $sale->total . '" maxlength="12" >';

          $content .= ' <input type="hidden" name="idsale" id="ClientId" value="' . $id . '" />';
    ...
    public function payaments($id) {
      ...
      $sale = Sale::find($id);
      $change = ($sale->total - $sale->paid) > 0 ? ($sale->total - $sale->paid) : '';
      $content = '<div class="row"><div class="col-md-12"><h4><b>' . label("Total") . '</b> ' . number_format((float)$sale->total, $this->setting->decimals, '.', '') . ' ' . $this->setting->currency . ' <b>&emsp;' . label("Paid") . ' :</b> ' . number_format((float)$sale->paid, $this->setting->decimals, '.', '') . ' ' . $this->setting->currency . '<b> &emsp; Saldo :</b> ' . number_format((float)($sale->total - $sale->paid), $this->setting->decimals, '.', '') . ' ' . $this->setting->currency . '</h4></div></div>';
   ...
   public function Addpayament($type){
      ...
      // actualizo calendar con el estado de abono
      // adiciona por lmrg
  /*     $cedula = $sale->numdoc;
       if ( $sale->status == '2')
          $estadores = '11';
      ...
      // $XOBJRES=new greservas;
       ...
       $checkinid = $sale->checkinid;
       $setid="001";

     //   $this->load->model('calendar_model', 'invoice');
         $calendar_details = calendar_details::find($sale->bookingid));

       $calendar_details->update_attributes(array(
             'status' => $estadores
         ));

         */
     /*    $XACTCALENDAR=$XOBJRES->actualiza_reservasale($bookindid, $estadores);
       $XACTCALENDAR=$XOBJRES->actualiza_checkinsale($checkindid, $estadores);
       */
