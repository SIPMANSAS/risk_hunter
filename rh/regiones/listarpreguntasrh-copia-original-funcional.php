...
$(document).ready(function() {
	...
	$.ajax({
            type: "POST",
            url: "funciones/obtienesearchpreguntaB.php",
            data: parametros,
            success: function(data) {
                //Escribimos las sugerencias que nos manda la consulta
                $('#sugerencia').fadeIn(1000).html(data);
...
if(isset($_POST["identificau"]) or (isset($_POST["idbloque"])))
{
    ...
    echo " <div class=\"contenedor\">
        ...
        <div class="buscar">
            ...
            <input type="search" id="searchB" placeholder="Buscar Encabezado..." autocomplete="off" />
        </div>
        <div id="sugerencia"></div>
        </div>
        <?php
        echo "<div class=\"titulo2\"><i class=\"fa-solid fa-user\"></i> Preguntas del Bloque</div>
       
        <div id=\"sugerenciaB\"></div>
        </div>
    ...
    while ($xrowp = $Json->obtener_fila($result)) {
        ...
        <div class=campos_pre>
            <form action="vervalores.php" method="POST" target=dek">
                <input type="hidden" name="preguntaid" value="<?php echo $identificadorpreg ?>">
                <input type="hidden" name="respuestaid" value="<?php echo $idrespuesta;?>">
                <button type="submit" class="btn_sel"><?php echo utf8_encode($nombreresp) ?></button>
            </form>
        ...
        <div class="campos_pre">
            <form action="listarpreguntash.php" method="POST" target="blank">
                <input class="ver" value="<?php echo utf8_decode($nombrepregunta) ?>" name="nombrepadre">
