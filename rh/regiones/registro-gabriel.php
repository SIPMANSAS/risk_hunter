...
if ((isset($_POST['enviar'])) && ($_POST['enviar'] == "Enviar"))
{
    ...
    if((!isset($xinsertu)) or ($xinsertu == FALSE)) {
        $correcto = "false";
        $disp .= "Se ha presentado un error en la creaci�n del usuario... ".$xinsertu;
    }
    else
    {
      ...
      if ($correcto == "false")
          $disp .= " Se ha presento error en la creaci�n del rol del usuario... ".$xinsertu;
      else 
      {
          ...
          if ($correcto == "false")
              $disp .= " Se ha presento error en la creaci�n de los Clientes a los que pertenece el usuario... ".$xinsertuc;
          else
          {
...
<body>  
    ...
    /* Inicia llamado del form con sus campos de nombre,apellido, email, telefono etc */
                ...
                echo '<form class="registro" action="registro1.php" method="post">'; 
                ...
                $xrowdominios = array();
                $xi=0;
                echo '<div class="inputs_r"><label for="identificacion">Identificacion</label><SELECT name="tipoident">';
                ...
                echo '</div>';
                if (isset($id))
                    echo '<label>Numero de identificación</label><input name="numident" type="text" value="'.$numident.'"  disabled>';
                else    
                    echo '<div class="inputs_r"><label>Numero de identificación</label><input name="numident" type="text" value="'.$numident.'"  required>';
                echo "</div>";
               ...
               $result = $Json->buscaidrolxusuario($filtro);
               $xrowrol = array();
               $i=0;
               while ($xrowrol = $Json->obtener_fila($result)) {
                   $idrolusuario[$i] = $xrowrol["id_rol"];
                  $i++;
               }
               ...
               $xrowcliente = array();
               $xi=0;
               echo '<div><legend>Clientes</legend>';
               ...
               echo '<div class="inputs_r"><input class="btn_gris campos" type="reset" name="limpiar" value="limpiar">   ';
               
               ?>
            <input class="btn_verde campos" type="submit" name="enviar" value="Enviar" onclick="return validateEmail()" /></div>
            <br><!-- Usuario Asignado -->
