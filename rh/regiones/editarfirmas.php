...
<div class="contenedor">
 ...
 /*       $datadep = $bloques->buscadepartamentos($paisid);       
        ...
        <div class="inputs_r"><label>Tipo:</label>
        <?php
        $filtrotipo = $tipoCliente;
        $datatipo = $bloques->buscatipo($filtrotipo);
        $traetipo = $bloques->obtener_fila($datatipo);
        
        $tipoT = $traetipo['nombre'];
        $idtipo = $traetipo['identificador'];

        $selectipo = $bloques->tiposelect($filtrotipo);
        ?>
        <select type="text" id="" name="tipoinspector">
            <option value="">Seleccionar...</option>
            <?php
            while($columna = $bloques->obtener_fila($selectipo)){
            ?>
            ...
            $selectTipo = "selected";
            ?>
            <option value="<?php echo $idtipo;?>"<?php echo $selectTipo?>><?php echo $tipoT ?></option>
        </select>
        </div>
...
<?php 
 ...
 //        foreach ($countries as $c) : 
              if($paisid == $c['codigo'])
              {
                echo " <option value=\"". $c['codigo'] ." \" selected>". utf8_encode($c['nombre']) ."</option>";    
              }
              else 
              {
