...
<body>
  ...
  <form class="registro" action="controller/controllerencabezado.php" method="POST" enctype="multipart/form-data">  
        ...
        <div class="inputs_r">
            ...
            echo $nombrecompania = $companixusuario['nombres'];
            */
            
            include 'conexion/conexion.php';
            $idusuario=$id_menu_p;
            $consultacompaniasxusuario = $mysqli->query("SELECT uc.id_cliente,ter.nombres,ter.apellidos nombre FROM sg_usuarios_x_cliente uc,ter_terceros ter WHERE	uc.id_cliente = ter.identificacion AND uc.id_usuario = '$idusuario' AND ter.tipocliente = 775");
            $extraerData=$consultacompaniasxusuario->fetch_array(MYSQLI_ASSOC);
            $companiaxusuario = $extraerData['nombres'];
            $idcliente = $extraerData['id_cliente'];
            
            ?>
            <input class="inp_med" type="" value="<?php echo $companiaxusuario?>" disabled>
             <input class="ver" type="" name="firmainspectora" value="<?php echo $idcliente?>">
        </div>
        <div class="inputs_r">
            <?php
            $consultatelefono = $mysqli->query("SELECT * FROM ter_terceros WHERE identificacion='$idcliente'");
            $extraerDatos = $consultatelefono->fetch_array(MYSQLI_ASSOC);
            $telefono = $extraerDatos['telefono'];
            ?>
            <label> Número de contacto de la firma inspectora:</label>
            <input class="inp_med" type="text" name="" placeholder="Ingrese Contacto Persona que Atiende" value="<?php echo $telefono ?>" disabled>
            <input class="ver" type="text" name="contacto_firma_inspectora" placeholder="Ingrese Contacto Persona que Atiende" value="<?php echo $telefono ?>" required>
        </div>
        <div class="inputs_r">
            <label>Oficina:</label>
                ...
                <select type="text" id="contact" name="tipoDoc" onkeyup="PasarValor();">
                  <option value="">Seleccionar....</option>
                    <?php while ($columna = $bloques->obtener_fila($consultatipodoc)) { ?>
                      <option value="<?php echo $columna['identificador']; ?>"><?php echo utf8_encode($columna['nombre']); ?> </option>
                <?php } ?>
        ...
        <div class="inputs_r">
            ...
            ?>
            <option value="<?php echo $columnai['identificador'];?>"><?php echo $columnai['id_usuario'].' - '.$columnai['nombre_corto'].' '.$columnai['apellidos'];?></option>
            <?php
            }
            ?>
            </select>
        ...
        <div class="inputs_r">
            ...
            <input class="inp_med" type="text" name="contacto_inspector" placeholder="Ingrese Contacto Inspector" required>
        </div>
        <div class="inputs_r">
            <?php
