  ...
  <form class="registro" action="controller/controllerencabezado.php" method="POST" enctype="multipart/form-data">  
        ...
        <div class="inputs_r">
            ...
            */
            ...
            $idusuario=$id_menu_p;
            $consultacompaniasxusuario = $mysqli->query("SELECT uc.id_cliente,ter.nombres,ter.apellidos nombre FROM sg_usuarios_x_cliente uc,ter_terceros ter WHERE	uc.id_cliente = ter.identificacion AND uc.id_usuario = '$idusuario' AND ter.tipocliente = 772");
            ...
            $consultaFirmasI = $bloques->buscafirmasI($idcliente); 
           
            ?>
            <input class="inp_med" type="" name="" value="<?php echo $companiaxusuario?>" disabled>
             <input class="ver" type="" name="companiaseguros" value="<?php echo $idcliente?>">
        </div>
        <div class="inputs_r">
            <label>Oficina:</label>
            <?php
            $consultaoficina = $mysqli->query("SELECT T.identificador,T.nombre FROM sg_usuarios U, ter_oficinas T WHERE U.identificador = '$idusuario' AND T.identificador = U.id_oficina");
            $extraerOficina = $consultaoficina->fetch_array(MYSQLI_ASSOC);
            $oficinaid = $extraerOficina['identificador'];
            $nombreoficina = $extraerOficina['nombre'];
            ?>
            <input class="inp_med" type="" name="" value="<?php echo $nombreoficina?>" disabled>
            <input class="ver" type="" name="oficina" value="<?php echo $oficinaid?>">
        ...
        <div class="inputs_r">
            <label>Técnico</label>
              <?php
              $consultatecnico = $bloques->buscauserxtecnicos();
