...
function BorrarRegistrocliente() {
 
	$('div.lista-cliente').each(function(index, item){
	    var idval = index + 1;
	    var idvalant = index;
	    
	 	jQuery(':checkbox', this).each(function () {
		       if ($("#itemcliente-" + idval).is(':checked')) {
                       $(item).remove();
                 }
	...
	$.ajax({
            ...
            success: function(data) {
                ...
                $('.sugerencia2-element').on('click', function(){
                        ...
                        //Obtenemos la id unica de la sugerencia pulsada
                        var id = $(this).attr('id');
                        var nomclicomp =  $('#'+id).attr('data') + ":" + id;
                        //Editamos el valor del input con data de la sugerencia pulsada
                         $('#nombrecliente-'+rowId).val(nomclicomp);
					    //Hacemos desaparecer el resto de sugerencias
                        $('#sugerencia2').fadeOut(1000);
                        alert('Has seleccionado el '+id+' '+$('#'+id).attr('data'));
                        return false;
...
if ((isset($_POST['enviar'])) && ($_POST['enviar'] == "Enviar"))
{
    ...
    if (isset($_POST["id"]) && ($_POST["id"] <> "") )
    {
        ...
        if (isset($roles[0]))
        {
            ...
            if((!isset($xborrau)) or ($xborrau == FALSE)) {
                ...
                /****************** SOLO SE COMENTARON LAS LINEAS 319 320 PARA LA VERSION ANTERIOR COMENTAR  
           LAS 342 Y 343 Y QUITAR EL COMENTARIO DE LAS 319 Y 320 (7/12/2022) ******************/
        ...
        if ($correcto == "false"){
            ...
            if (isset($clientes[0]))
            {
                ...
                if((!isset($xborrau)) or ($xborrau == FALSE)) {
                    $correcto = "true";
                    header("Location: listausuariosrh.php");
    ...
    else
    {
      ...
      }else 
      {
          ...
          if ($correcto == "false")
            $disp .= " Clientes actualizados correctamente".$xinsertuc;
...
<body>  
    ...
    /* Finaliza el encabezado del formulario incia el form formulario de captura de datos de usuario */
                  ...
                  echo '<div class="titulo_p"><i class="fa-solid fa-user"> </i>&nbsp;<div>Registro único de usuario</div></div>
                        <div class="titulo2"><i class="fa-solid fa-users"></i><a href="listausuariosrh.php"> Listar Usuarios</a></div>
    ...
    /* Inicia llamado del form con sus campos de nombre,apellido, email, telefono etc */
                ...
                if (isset($id) && ($id <> ""))
                    ...
                    echo '<div class="inputs_r"><label for="identificacion">Tipo de identificación</label><SELECT name="tipoident" required>';
                    echo '<option value="0">Seleccionar Tipo de Identificacion</option>';
                while ($xrowdominios = $Json->obtener_fila($xcompdominios)) {
                      $nombreidentificador[$xi] = ($xrowdominios['nombre']) ;
                ...
                echo '</select></div>';
                if (isset($id) && ($id <> ""))
                    echo '<div class="inputs_r"><label for="">Número de Identificación</label><input class="inp_med" name="numident" id="age" type="text" value="'.$numident.'"  disabled></div>';
                else    
                    echo '<div class="inputs_r"><label for="">Número de Identificación</label><input class="inp_med" name="numident" id="age" type="text" value="'.$numident.'"  required></div>';
                ...
                if (isset($id) && ($id <> ""))
                    ...
                    echo '<input class="inp_med" name="email" id="useremail" type="email" value=""  required>';
                 echo '</div>';
              ...
              /* Hago el select de roles */
               ...
               $resultc = $Json->buscarclienteusuario($filtro);
               $xrowcliente = array();
               $iclientexusuario=1;
               while ($xrowcliente = $Json->obtener_fila($resultc)) {
                   $clienteusuario[$iclientexusuario] = utf8_decode($xrowcliente["nombres"]);
                   $iclientexusuario++;
               }
...
// <a href="nuevosroles.php?id='.$id.'">Asignar Roles</a>
echo '          <div id="outerol">
                    <div id="listroles">';
...
<?php
            ...
            <div class="btn-action float-clear">
               ...
               <input class="btn_oscuro" type="button" name="borrar_registrosc" value="Borrar Registros" onClick="BorrarRegistrocliente();" /> ';
              ...
              echo '<div class="inputs_r">
                        ...
                        <input class="inp_med" name="oficina" type="text" value="" required>
                    </div>';      
