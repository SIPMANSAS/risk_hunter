...
?>
<head>
  <title>Asignación Nivel Prioridad</title>
...
<body>
  ...
  <div class="titulo_p"><i class="fa-solid fa-file-pen"></i> NIVEL DE PRIORIDAD </div>
  <div class="link_int">
        <div class="titulo2">
  ...
  <form class="registro" action="controller/controlleBienes.php" method="POST">
    <div class="inputs_r">
        ...
        $consultanivelP = $bloques-> consultanivelprioridad();
        ?>
    </div>
    
     <div class="inputs_r">
        <label>Nivel de Prioridad:</label>
        <input type="number" class="inp_med" name="prioridad" required>
        <?php
        /*
         <select type="text" id="tipobien" name="prioridad" onkeyup="PasaValor();" required>
            <option value="">Seleccionar....</option>
            <?php while ($columnaB = $bloques->obtener_fila($consultanivelP)) { ?>
              <option value="<?php echo $columnaB['identificador']; ?>"><?php echo utf8_encode($columnaB['nombre']); ?> </option>
        <?php } ?>
        </select>
        */
        ?>
    ...
    <div class="inputs_r">
      <input class="btn_azul" type='submit' onclick="location.reload()" name="asignaprioridad" value="Guardar">
    </div>
  </form>
