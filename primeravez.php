...
if ((isset($_POST['guardar'])) && ($_POST['guardar'] == "Guardar"))
{
    ...
    //{
     //if(preg_match('/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[$@$!%*?&#.$($)$-$_])[A-Za-z\d$@$!%*?&#.$($)$-$_]{8,15}$/',$password1))
        ...
        $filtro = "email = '$email'";
        $setval = "password='$password1', flag_primera_vez='1', estado='1'  ";    
        ...
        if((!isset($passupdate)) or ($passupdate == FALSE)) {
            ...
            $hash = md5($email);
            $lnk = "https://desarrollosysolucionesingenieriles.com.co/risk_hunter/";
...
<body>  
    ...
    <?php
                ...
                $consultapreviacorreo = $mysqli->query("SELECT * FROM sg_usuarios WHERE usuario LIKE '%$usuario%'");
