       ...
       $filtro = "descripcion like \"%".$key."%\"";
	    ...
	    $xi=0;
	     while ($xrowproyectos = $Json->obtener_fila($xcompproyectos)) {
           ...
           $html .= '<div><a class="sugerencia-element" data="'.($xrowproyectos['descripcion']).'" tipod="'.$xrowproyectos['vdom_tipo_dato'].'" id="'.$xrowproyectos['identificador'].'">'.utf8_encode($xrowproyectos['descripcion']).'</a></div>';
        }
       echo $html;
  }
  else 
