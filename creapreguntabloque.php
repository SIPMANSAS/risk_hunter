...
function insertarpregunta($idbloque, $nom_pregunta, $nom_respuesta, $respuesta_cierre, $respuesta_riesgo, $id_padre, $id_respuesta, $estadoval )
{
    ...
    for ($i=0; $i < count($nom_pregunta); $i++)      
    {
            ...
            else
            {
             ...
             if (isset($respuesta_cierre[$i]) && ($respuesta_cierre[$i] != NULL))
                $xpregunta[6] = $respuesta_cierre[$i];
             else 
                 $xpregunta[6] = NULL;
             $xpregunta[7] = $idbloque;
             if (isset($respuesta_riesgo[$i]) && ($respuesta_riesgo[$i] != NULL))
...
if (isset($_GET["id"]) or (isset($_POST["id"])))
{
         if (isset($_GET["id"]))
            ...
            //llamado del id desde un bloque
            //$id = $_POST["id"];
            /* Llamado desde una pregunta que se encuentra dentro de un bloque */
...
{   
    ...
    if (isset($id) && ($id > 0))
    {
        ...
        window.location.href="creapreguntabloque.php"</script>';
    ...
    $xsequencia = $Json->buscarsecuenciaglobal($tabla, $identificador);
        ...
        /* luego procedo a insertar las preguntas */
        list ($correcto, $disp2) = insertarpregunta($xidbloquepregunta, $_POST["pro_nom"], $_POST["nom_respuesta"], $_POST["respuestacierre"], $_POST["respuestariesgo"], $_POST["pro_padre"], $_POST["id_respuesta"], 
        $_POST["estadoval"]);
        $disp = $disp."<br>".$disp2;
          }
}
...
include 'header-rh.php';
    ...
    echo '             
                        ...
                        <div class="link_int">
                        <div class="titulo2"><i class="fa-solid fa-list"></i><a href="listarpreguntasrh.php"> Listar Bloques de Preguntas</a></div>
                        <div class="titulo3"><i class="fa-solid fa-question"></i><a href="creapreguntabloque.php"> Registro Bloque de Preguntas</a></div></div>
...
/* ingreso campos del formulario de bloque de preguntas */
    ...
    echo '<div class="switch-button"><legend></legend>';
    if (isset($estado))
        if ($estado == "on")
   ...
   if (isset($_GET["id"]) or (isset($_POST["id"])))        
    {
        ...
        /**********CAMBIOS SEBASTIAN 02/12/2022 **********/
            $idpr = $_POST["id"];
            //$filtro = "p.id_bloque_preguntas = '$idpr'";
            $filtro = "p.identificador='$idpr' ";
            $result = $Json->buscarpreguntas($filtro);
            $xrowp = array();
             $xpreg = 0;
        //////// CAMBIOS SEBASTIAN 02/12/2022 //////////
...
//        </div>';
        ...
        <div class="campos_res titulo">Nombre Pregunta</div>
        <div class="campos_res titulo">Pregunta Padre</div>
        <div class="campos_res titulo">Respuesta</div>
        <div class="campos_res titulo">Respuesta  Cierre</div>
        <div class="campos_res titulo">Respuesta  Activa Riesgo</div>
        <div class="campos_res titulo">Estado</div>
        <div class="campos_res titulo">Gestionar Riesgo</div>                    
        </div>';
//fin edicion gabriel
        ...
        echo $identificadorpreg =  $xrowp["identificador"];
        ...
        echo "<div class=\"campos_res\">$nombrepregunta</div>";
        echo "<div class=\"campos_res\">".utf8_encode($nompadre)."</div>";
        echo "<div class=\"campos_res\">".($nombreresp)."</div>";
                
        echo "<div class=\"campos_res\">$nomcierre</div>";
        echo "<div class=\"campos_res\">$nomriesgo</div>";
        //echo "<div class=\"campos_list_u\">$fecha_efectiva </div>";
        //fin edicion gabriel
        echo '<div class="estado campos_res">
        ...
        echo "<div class=\"campos_res\"><input type=\"submit\"  class=\"btn_sel\" name=\"adicionariesgo\"
